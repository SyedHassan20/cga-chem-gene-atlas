...
# File path defined
file_path <- "/Users/hassan/Desktop/UCSF Lab Work/CGA App/data/merged_data.rds"
    ...
    fluidRow(
      ...
      column(
        ...
        div(
          ...
          br(),
          tags$a(
            href = "mailto:marina.sirota@ucsf.edu",
            target = "_blank",
            "Contact Us"
          ),
          br(),
          ...
          tags$a(
            href = "https://github.com/SyedHassan20/cga-chem-gene-atlas",
            target = "_blank",
            "GitHub"
          )
        )
...
server <- function(input, output, session) {
  ...
  # Define custom palette for first 3 donuts
  my_pastel_colors <- c(
  ...
  # Define more diverse blue palette for the 4th donut
  my_blues <- c(
    "#1B3768", "#174A8F", "#1C6FBD", "#2986CC")
  
  # Load merged data
  if (file.exists(file_path)) {
    merged_df_all <- readRDS(file_path) %>%
      mutate(
  ...
  merged_df_all <- merged_df_all %>%
    rename(
      ...
      `2nd`     = X2nd.trimester,
      `3rd`     = X3rd.trimester,
      Postnatal = Postnatal.lethality
    )
  
  colnames(merged_df_all)
  # Replace NA in main filter columns
  merged_df_all <- merged_df_all %>%
    mutate(
  ...
  # Master filter function (no memoise)
  filteredData <- function(gf, sa, lm, lt) {
    ...
    if (!is.null(lt) && lt != "All") {
      ...
      if (!is.null(col_name) && col_name %in% names(data)) {
        data <- data %>% filter(!is.na(.data[[col_name]]) & .data[[col_name]] != "")
      }
    }
    data
  }
  ...
  lethalityTimingData <- reactive({
    df_lethality_timing <- filteredData(
      reactiveFilters$selectedGeneEffect,
      reactiveFilters$selectedSystemAffected,
      reactiveFilters$selectedLethalityMode,
      reactiveFilters$selectedLethalityTiming
  ...
  observeEvent(event_data("plotly_click", source = "lethalityTimingChart"), {
    ...
    if (!is.null(ed$customdata)) {
      selected_timing <- as.character(ed$customdata)
      reactiveFilters$selectedLethalityTiming <- selected_timing
      updateSelectInput(session, "lethalityTimingSelect", selected = selected_timing)
    }
  })
  ...
  output$dataTable <- renderDataTable({
    ...
    # Rename only existing columns safely
    df <- setNames(df, c(
      ...
      "Human Evidence", "Intolerome PubMed IDs", "1st", "2nd", 
