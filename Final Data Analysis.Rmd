...
ctd[, InteractionActions := gsub("\\^", " ", InteractionActions)]
ctd[, InteractionActions := gsub("\\|", "; ", InteractionActions)]
...
intolerome[, Gene_Locus := gsub(";$", "", Gene_Locus)]

# Split into name and ID
intolerome[, c("Gene_Name", "Gene_ID") := tstrsplit(Gene_Locus, "; id:")]

# Clean new columns
intolerome[, Gene_Name := gsub("name: ", "", Gene_Name)]
intolerome[, Gene_ID := as.numeric(Gene_ID)]

# Drop old Gene_Locus
intolerome[, Gene_Locus := NULL]

# Move Gene_Name and Gene_ID to the start
setcolorder(intolerome, c("Gene_Name", "Gene_ID", setdiff(names(intolerome), c("Gene_Name","Gene_ID"))))
...
```{r}
# --- Merge Files ---
# Force join keys to the same type
head(ctd$InteractionActions, 20)

intolerome <- intolerome %>% mutate(Gene_ID = as.character(Gene_ID))
ctd <- ctd %>% mutate(GeneID = as.character(GeneID))
# Now join
merged_df <- left_join(
  intolerome,
  ctd,
...
print(merged_df)
head(merged_df$InteractionActions, 20)
...
plot_distribution_unique_chemicals <- function(data, column_name, title, 
  ...
  if (use_percent) {
    plot <- ggplot(distribution, aes(x = reorder(!!sym(column_name), -Percent), y = Percent)) +
      geom_bar(stat = "identity", fill = fill_color, color = border_color) +
      # ✅ "<1" for very small values, whole number for everything else
      geom_text(aes(label = ifelse(Percent < 1, "<1", round(Percent, 0))), 
                vjust = -0.3, size = 5) +
      labs(
        title = title,
        x = formatted_column_name,
        y = "% of Unique Chemicals"
      )
  } else {
    plot <- ggplot(distribution, aes(x = reorder(!!sym(column_name), -Count), y = Count)) +
      geom_bar(stat = "identity", fill = fill_color, color = border_color) +
  ...
  plot +
    theme_minimal() +
    theme(
      axis.text.x   = element_text(size = 12.5, angle = 90, hjust = 0.8, margin = margin(t = 0)),
      plot.title    = element_text(hjust = 0.5, face = "bold"),
      axis.text.y   = element_text(size = 13),
      axis.title.x  = element_text(size = 15, margin = margin(t = -1)),  # smaller top margin
...
# Gene Effect (percentages, excluding blanks)
p1 <- merged_df %>%
  mutate(Gene.Effect = str_trim(Gene.Effect)) %>%       # trim spaces
  filter(Gene.Effect != "" & !is.na(Gene.Effect)) %>%   # remove blanks / NAs
  plot_distribution_unique_chemicals(
    column_name  = "Gene.Effect",
    title        = "Distribution of Gene Effect (Unique Chemicals)",
    fill_color   = "lavender",
    border_color = "black",
    use_percent  = TRUE
  )

# Allowed categories for System.Affected (from your figure)
allowed_systems <- c(
  "Multiple >3", "Metabolic", "Cardiovascular", "Neurologic", 
  "Skeletal", "Immunologic", "Hematopoietic", "Renal", 
  "Neuromuscular", "Genitourinary", "Muscular", "Skin", 
  "Respiratory", "Reproductive", "Gastrointestinal", "Placental", 
  "Other", "Facial dysmorphism", "Ciliopathic"
)

p2 <- merged_df %>%
  # normalize text
  mutate(System.Affected = str_trim(tolower(System.Affected))) %>%
  # recode variants
  mutate(
  ...
  filter(System.Affected %in% allowed_systems) %>%
  mutate(System.Affected = factor(System.Affected, levels = allowed_systems)) %>%
  # plot
  plot_distribution_unique_chemicals(
...
# Lethality Mode of Function (percentages, excluding blanks)
p3 <- merged_df %>%
  mutate(Lethality.Mode.of.Function = str_trim(Lethality.Mode.of.Function)) %>%     # trim
  filter(Lethality.Mode.of.Function != "" & !is.na(Lethality.Mode.of.Function)) %>% # remove blanks / NAs
  plot_distribution_unique_chemicals(
    column_name  = "Lethality.Mode.of.Function",
    title        = "Distribution of Lethality Mode of Function (Unique Chemicals)",
...
# -----------------------------
# Lethality Timing (keep counts)
lethality_timing_long <- merged_df %>%
  rename(
    "Trimester 1st" = X1st.trimester,
    "Trimester 2nd" = X2nd.trimester,
    "Trimester 3rd" = X3rd.trimester,
    "Post natal" = Postnatal.lethality
  ) %>%
...
# -----------------------------
# Display plots
ggsave("Gene_Effect_Distribution.png", p1, width = 8, height = 6, dpi = 600)
ggsave("System_Affected_Distribution.png", p2, width = 8, height = 6, dpi = 600)
ggsave("Lethality_Mode_Distribution.png", p3, width = 8, height = 6, dpi = 600)
#ggsave("Lethality_Timing_Distribution.png", p4, width = 8, height = 6, dpi = 600)
print(p1)
print(p2)
print(p3)
print(p4)
...
# Save the Venn diagram for Genetics in Medicine (single column, 3.5 × 3.5 in, 600 dpi)
ggsave(
  filename = "/Users/hassan/Desktop/UCSF Lab Work/CGA App/venn_diagram.png",
  plot = venn.plot,
...
p <- ggplot(chemicals, aes(x = reorder(str_to_title(X..ChemicalName), n), y = n)) +
...
barplot(reactome_chem_results, showCategory = 10, title = "Reactome Enrichment (BPA & B[a]P Genes)") +
  theme(
    plot.margin = margin(t = 10, r = 30, b = 10, l = 50),  # increase left margin
