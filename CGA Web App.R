...
server <- function(input, output, session) {
  ...
  my_pastel_colors <- c("#19376D", "#215EA8", "#3185CE", "#4DA1DE", 
                        "#62B6CB", "#79C7B1", "#90D897", "#A8E97F",
                        "#C1D1E4", "#A7B5D9", "#8E9ACF", "#767FC5",
                        "#D2B48C", "#B8A078", "#9F8C64", "#E87A71")
  
  # 2) Define more diverse blue palette for the 4th donut
  my_blues <- colorRampPalette(c(
    "#011f4b", "#03396c", "#005b96", "#6497b1", "#b3cde0",
    "#8cb2cc", "#6699cc", "#3377cc", "#0066cc", "#004c99"
  ))(20)
  ...
  generate_donut_chart <- function(count_data, label_col, title, chart_source, color_palette) {
    ...
    plot_ly(
      ...
      source    = chart_source,
      marker    = list(colors = color_palette, line = list(color = "white", width = 1)),
  ...
  output$geneEffectChart <- renderPlotly({
    ...
    generate_donut_chart(df, "Gene.Effect", "Gene Effect",
                         "geneEffectChart", my_pastel_colors)
  })
  ...
  observeEvent(input$zoomGeneEffect, {
    showModal(modalDialog(
      ...
      renderPlotly({
        df <- geneEffectData()
        generate_donut_chart(df, "Gene.Effect", "Gene Effect",
                             "geneEffectChartZoomed", my_pastel_colors) %>%
          layout(height = 600)
      }),
  ...
  output$lethalityModeChart <- renderPlotly({
    ...
    generate_donut_chart(df, "Lethality.Mode.of.Function", 
                         "Lethality Mode of Function",
                         "lethalityModeChart", my_pastel_colors)
  })
  ...
  observeEvent(input$zoomLethalityMode, {
    showModal(modalDialog(
      ...
      renderPlotly({
        ...
        generate_donut_chart(df, "Lethality.Mode.of.Function",
                             "Lethality Mode of Function",
                             "lethalityModeChartZoomed", my_pastel_colors) %>%
          layout(height = 600)
      }),
      easyClose = TRUE, size = "l"
    ))
  ...
  output$dataTable <- renderDataTable({
    ...
    # Remove OMIM Gene (Duplicate) & Gene Symbol from display
    df <- df %>%
      select(
        `Gene Name`, `Gene ID`, `OMIM Gene`, `Gene Effect`, `Variants`,
        `Lethality Mode of Function`, `Coordinates (hg19)`, `Disease`,
        `OMIM Phenotype`, `Inheritance`, `System Affected`,
        `Prenatal Phenotypes`, `Fetal Sex`, `Human Evidence`, `PMID`,
        `1st`, `2nd`, `3rd`, `Postnatal`,
        `Chemical Name`, `Chemical ID`, `CAS RN`, `Gene Forms`,
        `Organism`, `Organism ID`, `Interaction`, `Interaction Actions`,
        `PubMed IDs`
      )
    
    # Force certain columns to character
    problematic_cols <- c("Gene Name", "Gene ID", "OMIM Gene", "Gene Effect", 
                          "Disease", "Variants", "Fetal Sex", "PMID", 
                          "1st", "2nd", "3rd", "CAS RN", "Gene Forms")
    
    df <- df %>%
      mutate(across(all_of(problematic_cols), as.character))
    
    # Convert PMID & PubMed IDs into hyperlinks
    ...
    if ("PMID" %in% colnames(df)) {
      df$PMID <- sapply(df$PMID, make_pubmed_links)
    }
    if ("PubMed IDs" %in% colnames(df)) {
      df$`PubMed IDs` <- sapply(df$`PubMed IDs`, make_pubmed_links)
    }
